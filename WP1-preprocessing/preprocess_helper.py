...
def remove_traj(df_point,t):
    ...
    list_occurence = df_point.loc[df_point.loc[:,'dt'] > t].index
    ...
    traj_remove = np.where(traj_times < t)[0]
...
def spline_coeff(lat, time):
    ...
    if lat[~np.isnan(lat)].shape[0] > 5:
        save_data = optimal_knots(lat[~np.isnan(lat)], time)
        t2 = time[save_data[1:-1]]
        try2 = LSQUnivariateSpline(time, lat[~np.isnan(lat)], t2, k=3)
        spl_coeff = try2.get_coeffs()
        return [spl_coeff, t2]  # in this case returns spline
    else:
...
def smoother(df_point):
    ...
    init_columns = ['icao', 'phase', 't_start', 't_end', 'lat', 'lon', 'alt', 'gs', 'trk', 'roc']
    ...
    for i in df_point.index.unique():
        ...
        new_df.loc[counter_rows, 't_start'] = spec_traj.loc[:,'ts'].values[0]
        new_df.loc[counter_rows, 't_end'] = spec_traj.loc[:, 'ts'].values[-1]
